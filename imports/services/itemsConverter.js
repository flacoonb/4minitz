...
export class ItemsConverter {
  static async convertItem(infoOrActionItem) {
    if (infoOrActionItem instanceof ActionItem) {
      ...
      // If optional isOpen attribute is not yet available, we must set it to
      // true, otherwise the AI will be considered "done" immediately after
      // conversion. So, a "done" state is preserved when converting
      // back-and-forth.
      ...
      // are not allowed to have isSticky attribute, we set it to false here,
      // without condition. So, the isSticky state is lost when converting
      // back-and-forth
