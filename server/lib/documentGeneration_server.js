...
convertHTML2PDF = (htmldata, fileName, metaData) => {
  ...
  const checkFileExists = (filepath, fileNameForErrorMsg) => {
    if (!fs.existsSync(filepath)) {
      throw new Meteor.Error(
          "runtime-error",
          `Error at PDF generation: ${fileNameForErrorMsg} not found at: ${
              filepath}`,
      );
    }
  };
  ...
  if (Meteor.settings.public.docGeneration.format === "pdfa") {
    ...
    additionalArguments =
        ...
        ` -dColorConversionStrategy=/${icctype} -sProcessColorModel=Device${
            icctype} -sDEVICE=pdfwrite -dPDFACompatibilityPolicy=1 -sOutputFile=`;
...
if (Meteor.settings.docGeneration?.enabled) {
  ...
  const settingsPath =
      createDocumentStoragePath(undefined); // eslint-disable-line
