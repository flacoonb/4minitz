...
export class MigrateV24 {
  ...
  static fixItem(infoOrActionItem) {
    ...
    const fixedDetails = infoOrActionItem.details.map(
        (detail) => this.fixDetails(detail),
    );
  ...
  static down() {
    ...
    // Due to the fact that initially migration #23 did not find() minutes on
    // the collection but on the MinutesSchema the return value was not a plain
    // minutes document. Instead it contained astronomy classes, e.g.
    // InfoItemSchema in topics.$.infoItems. This breaks the database. Migration
    // #23 is fixed now and we don't want to break database when downgrading
    // from #24 to #23.
  }
}
