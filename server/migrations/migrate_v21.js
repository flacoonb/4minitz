...
export class MigrateV21 {
  static up() {
    ...
    console.log(
        "Migration V21 recovers info items which are missing in the topics collection due to a bug " +
...
class MinutesHandler {
  constructor() { this.itemsDictionary = {}; }
  ...
  _updateTopicsOfMeetingSeries(series) {
    ...
    const topicsInCollection = TopicsFinder.allTopicsOfMeetingSeries(
        series._id,
    );
    topicsInCollection.forEach((topicInCollection) => {
      topicInCollection.infoItems.forEach(
          (item) => { delete this.itemsDictionary[item._id]; });
      const missingItems = this._getMissingItemsForTopic(topicInCollection._id);
      if (missingItems.length > 0) {
        topicInCollection.infoItems =
            topicInCollection.infoItems.concat(missingItems);
        TopicSchema.getCollection().update(topicInCollection._id, {
    ...
    console.log(
        `MigrationV21: Recovered ${recoveredItemsCounter} missing items`,
    );
  }

  _getMissingItemsForTopic(topicId) {
    const missingItems =
        Object.keys(this.itemsDictionary)
            .filter((key) =>
                        this.itemsDictionary[key].parentTopicId === topicId)
            .map((key) => {
